...
class CarDetection:
    ...
    def validate_indian_license_plate(self, text): #TO CHECK THAT TEXT WE READ FOLLOW THE PATTERN OF INDIAN NUMBER PLATE
        patterns = [
            re.compile(r'^[A-Z]{2}\s?\d{2}\s?[A-Z]\s?\d{4}$'),#BR 11 L 3141
            re.compile(r'^[A-Z]{2}\s?\d{2}\s?[A-Z]{1,2}\s?\d{4}$') #BR 11 AB 3141
        ]
        return any(pattern.match(text) for pattern in patterns)

    def correct_common_errors(self, text):
        text = text.strip()
        text = re.sub(r'^[^\w]+', '', text)#RE LIB HAS BEEN USED HERE,JO UPR IMPORT KIYE HAI NA--REMOVING ALPHANUMERIC CHAR AT START OF STRING
    ...
    def lens_api(self, frame):
        try:
            # Convert BGR to RGB (optional - depends on your OCR requirements)
    ...
    def plot_boxes(self, results, frame):
        ...
        for i in range(n):
            ...
            if row[4] >= 0.2:
                ...
                data = self.lens_api(cropped_img)
                print(data)
                if not "error" in data:
                    if "text_segments" in data and len( data["text_segments"]) > 0:
                        data = data["text_segments"][0]
                        if data[0] == '"' or data[0] == '*':
                           data = data[1:]
                        corrected_text = data
                        if len(corrected_text) > 4:
                            if self.validate_indian_license_plate(corrected_text):
                                ...
                                with open('output.csv', 'r') as file:
                                    olcsv = file.read()
                                if corrected_text not in olcsv:
                                    self.write_to_csv(corrected_text)
                # for r in data["text_segments"]:
                #     print(r)  
                ...
                #         print(f"THE LICENCE PLATE DETECTED IS : {corrected_text}")
                #         if self.validate_indian_license_plate(corrected_text):
                #             text_x, text_y = x1, y1 - 10
                #             cv2.putText(frame, corrected_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                #                         (0, 0, 255), 2)
...
if __name__ == "__main__":
    CarDetection(capture_index='3mov.mp4', model_name='best.pt')
